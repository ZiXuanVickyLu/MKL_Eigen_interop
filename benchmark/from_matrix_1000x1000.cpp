...
bool test_solve() {
    ...
    from_file(triplet, prefix + "N1000.matrix");
    from_file(triplet_m, prefix + "mass1000.matrix");
    data_from_file(rhs, prefix + "rhs1000.data");
    data_from_file(lhs, prefix + "lhs1000.data");
    ...
    auto ieSystem = N + 1.0/(1e-2*1e-2) * M;
    ...
    Eigen::VectorXd c = solver.solve(rhs_eigen);

    std::cout << "SimplicialLLT solve time: " << static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds >(std::chrono::steady_clock::now() - t1).count())/ 1000.0  << " ms " << std::endl;
    ...
    std::cout << "PardisoLLT factorize time: " << static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - t2).count())/ 1000.0  << " ms " << std::endl;

    auto t3= std::chrono::steady_clock::now();
    VectorXd c2 = pardisoSolver.solve(rhs_eigen);
    std::cout << typeid(c2).name() << std::endl;
    std::cout << "PardisoLLT solve time: " << static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - t3).count())/ 1000.0
    << " ms " << std::endl;
